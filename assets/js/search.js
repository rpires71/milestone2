...
function filterPlaces(placeType, button) {
    ...
    if (cityInput.value.trim() !== '') {
        ...
        performSearch(cityInput.value.trim()); // Trigger a new filtered search
    } else {
...
async function searchNearbyPlacesNew(location) {
    ...
    try {
        ...
        const { places } = await google.maps.places.Place.searchNearby({
            locationRestriction: {           // Defines search area
                center: {
    ...
    places.forEach((place, index) => {
        ...
        resultItem.addEventListener('click', function () {
            if (place.location) {
                ...
                if (markers[index]) {
                    if (markers[index].setAnimation) { // Ensure animation is supported
    ...
    const infowindow = new google.maps.InfoWindow({
        content: `
            <div style="padding: 10px; max-width: 200px;">
                ...
                ${
                    place.rating
                        ? `<p style="margin: 5px 0 0 0; color: #FF6B35; font-weight: bold;">⭐ ${place.rating.toFixed(1)}</p>`
                        : ''
                }
            </div>
        `
...
function clearMarkers() {
    ...
    markers.forEach(marker => {
        if (marker.setMap) {       // Ensure marker has a valid setMap() method
